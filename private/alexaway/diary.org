...
***** Block assembly
      Block assembly算法就是在上述图中选取一些边, 让这些边的分数总和最高, 而且保持仍然为连通图, 每一个block都可以计算出唯一的一个到其他任意一个block的变换.
      定义 B = {b_1,..., b_m} 为M个输入的block
      定义 X = {x_1,..., x_n} 为N个匹配的成对的portal或成对的block的二元集合
****** Matching confidence 
       该项定义了平均matching score, 这一项鼓励block的对齐结果更加紧凑
       [[file:diary.org_imgs/20161207_191810_19583hiB.png]]
       其中s_i 表示第i条边的Matching score, x_i 表示第i条边是（1）否（0）选中
****** Connectivity
       为了能使更多的block能够匹配, 这里鼓励有更多的边
       [[file:diary.org_imgs/20161207_192506_19583usH.png]]
      为了保证全局一致性, 本文添加了两个具有排他性的限制：其一是一个portal不能与多个portal对齐, 其二是一个block不能穿过另一个block的空间,如下图所示
      [[file:diary.org_imgs/20161207_200905_19583IBU.png]]
      总的约束就是：
      [[file:diary.org_imgs/20161207_201830_19583VLa.png]]
      其中P表示所有的portal, i ∈ N(p_j) 表示与portal P_j 相连的邻居 portal, 空心P表示有conflict的对齐方案的集合
***** Acceleration algorithm
      如果要枚举所有情况, 将耗费大量时间,本文共做了两项优化,第一是检测冲突只是在很少数量的blocks之间, 第二是本文引入了‘conflict tolerance’作为一个软约束
****** Constraint set
       本文加速方法依赖于这样一个事实：静止局部冲突有利于全局配准的一致性
****** Conflict tolerance
       如果大量噪声或者有很多局外点的时候, 冲突检测就会出错. 在冲突检测中会出现两种错误：一种是本来冲突的一些alignment被判定为不冲突的, 本文认为这种情况问题不大, 因为这种情况置信度一般不高. 在全局优化的过程中, 这种问题会被优化掉. 第二种是本来是好的alignment被判定为冲突的, 为了解决这类问题, 本文提出了一个tolerance的数据项：
       [[file:diary.org_imgs/20161207_213011_3720paw.png]]
       其中v_i 表示当由第i条边连接的两个block冲突的区域的体积, vol（b_j）表示b_j 的体积
      所以最终的式子是：
      [[file:diary.org_imgs/20161207_214449_37202k2.png]]
...
****** 高斯混合模型——GMM（Gaussian Mixture Model） 
   ...
   :LOGBOOK:
   CLOCK: [2016-12-07 Wed 20:01]--[2016-12-07 Wed 21:06] =>  1:05
   CLOCK: [2016-12-07 Wed 18:44]--[2016-12-07 Wed 19:25] =>  0:41
...
**** gitlab repo 默认位置 
     ...
     :LOGBOOK:
     CLOCK: [2016-12-09 Fri 16:04]--[2016-12-09 Fri 16:05] =>  0:01
     :END:
   [2016-12-09 Fri 16:04]
**** 三角形重心坐标 
     已知三角形的三个顶点的坐标,重心坐标是三个顶点的平均值
     :LOGBOOK:
     CLOCK: [2016-12-09 Fri 18:02]--[2016-12-09 Fri 18:03] =>  0:01
     :END:
   [2016-12-09 Fri 18:02]
*** 2016-12-15 Thursday
**** Matlab 笔记 
****** [c d]=components(A) 
       计算图A的连通分量, 其中A必须为稀疏矩阵, c返回每个顶点属于哪一个分量, d返回每个连通分量的大小

     :LOGBOOK:
     CLOCK: [2016-12-15 Thu 16:08]--[2016-12-15 Thu 16:10] =>  0:02
     :END:
   [2016-12-15 Thu 16:08]
*** 2016-12-19 Monday
**** 一个计算两个平面交线的算法 
     [[file:files/intersection.doc][Intersection between two planes]]
     :LOGBOOK:
     CLOCK: [2016-12-19 Mon 21:31]--[2016-12-19 Mon 21:33] =>  0:02
     :END:
   [2016-12-19 Mon 21:31]
*** 2016-12-20 Tuesday
**** 一个关于off文件参数设置的html文档 [[file:files/Appearances%20-%20Geomview%20Manual.html][戳这里]] 
     :LOGBOOK:
     CLOCK: [2016-12-20 Tue 17:36]--[2016-12-20 Tue 17:37] =>  0:01
     :END:
   [2016-12-20 Tue 17:36]
*** 2016-12-30 星期五
**** 《星星之火可以燎原》节选 
     ...
     :LOGBOOK:
     CLOCK: [2016-12-30 五 00:17]--[2016-12-30 五 00:18] =>  0:01
     :END:
   [2016-12-30 五 00:17]
* 2017
** 2017-01 一月
...
***** 添加用户
      #+BEGIN_SRC shell
        useradd -m alexaway  # -m 表示添加用户目录
        sudo passwd alexaway # 初始用户是没有密码的，必须为之分配密码，否则无法登录
      #+END_SRC
***** 关于远程硬盘
      #+BEGIN_SRC shell
        sudo apt-get install sshfs
        sshfs sshfs -o allow_other,reconnect,ServerAliveInterval=15,ServerAliveCountMax=3 用户名@192.168.1.103:/data/ddata /mnt/dir
      #+END_SRC


     :LOGBOOK:
     CLOCK: [2017-01-02 一 22:37]--[2017-01-02 一 22:45] =>  0:08
     :END:
   [2017-01-02 一 22:37]
*** 2017-01-07 星期六
**** spacemacs 代码折叠
     ...
     z m：关闭所有
     :LOGBOOK:
     CLOCK: [2017-01-07 六 17:17]--[2017-01-07 六 17:18] =>  0:01
     :END:
   [2017-01-07 六 17:17]
*** 2017-01-09 Monday
**** 在多个文件中寻找 M-x grep-find 
     :LOGBOOK:
     CLOCK: [2017-01-09 Mon 22:23]--[2017-01-09 Mon 22:24] =>  0:01
     :END:
   [2017-01-09 Mon 22:23]
*** 2017-01-14 星期六
**** linux 有趣的命令 
***** cmatrix
      #+BEGIN_SRC sh
